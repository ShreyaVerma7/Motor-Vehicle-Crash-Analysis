...
head(crash)
```
We used the dataframe above to create a bar chart for the percentage of fatal accidents.
...
x <- x %>% group_by(CONTRIBUTING.FACTOR.VEHICLE.1) %>% 
    summarise(NUMBER.OF.PERSONS.TOT.HURT = sum(NUMBER.OF.PERSONS.TOT.HURT),.groups = 'drop')
